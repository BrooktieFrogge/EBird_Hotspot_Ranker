...
- main purpose: calculate 'wtd-rf' and 'rfpc'.

---
1. wtd-rf (weighted rank factor)
---
- WHAT:
        a score for each bird. it's a weighted average
        of its observation frequency, how many times it was reported seen.
        uses a 0-1 scale.

- WHY:
        it gives more importance to observations from weeks with a large
        sample size and less to weeks with a small sample size. this prevents outlying
        rare sightings from skewing the rank. the 'weight' is the amount of checklists submitted aka
        the sample size.

- HOW:
        the formula is: wtd-rf = (freq_w1 * (weight_w1 / total_weight)) + (freq_w2 * (weight_w2 / total_weight)) + ...
        steve does it cell by cell in excel for each bird
        pandas does it in two lines for every bird at the same time:
        weighted_sum = (df[cols_to_keep] * weights_to_use).sum(axis=1)
        df['wtd-rf'] = weighted_sum / total_weight if total_weight > 0 else 0 

---
2. rfpc (rank factor percent)
---
- WHAT:
        this compares every bird's score to the #1 ranked bird.

- WHY:
        it makes the wtd-rf score easy to understand. the top bird
        always gets 100%. a bird with an rfpc of 50% means its weighted
        score was exactly half of the top bird's.

- HOW:
        after sorting and getting the top bird:

        rank just = sorted wtd-rf,

        max_wtd_rf = final_df['wtd-rf'].iloc[0]
        final_df['rfpc'] = (final_df['wtd-rf'] / max_wtd_rf) * 100
