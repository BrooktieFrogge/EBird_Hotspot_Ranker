...
'''
##### imports
import server.routes.rankings as rankings 
...
def main():
    ...
    while True:
        ...
        with sync_playwright() as p:
            BROWSER = p.chromium.launch(headless=rankings.HEADLESS)

            # ensure session is valid, autologin if needed
            rankings.ensure_session(BROWSER)

            for loc in process_list:
                print(f"[info] | fetching {loc} data from ebird...")
                raw_data = rankings.fetch_data(BROWSER, loc, start_yr, end_yr)
